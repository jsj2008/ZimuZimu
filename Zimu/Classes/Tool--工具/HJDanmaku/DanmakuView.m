...
////    _danmakuRenderer = [[DanmakuRenderer alloc] initWithCanvas:self configuration:_configuration];
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (DanmakuSource *danmakuSource in items.objectEnumerator) {
            if ([danmakuSource isKindOfClass:[DanmakuSource class]]) {
                DanmakuBaseModel *danmaku = [DanmakuFactory createDanmakuWithDanmakuSource:danmakuSource configuration:self.configuration];
                if (danmaku) {
                    [danmakus addObject:danmaku];
                }
            }
...
- (void)onTimeCount
{
    ...
    BOOL isBuffering = [self.delegate danmakuViewIsBuffering:self];
    if (self.curDanmakus == nil) {
        NSLog(@"没了，停");
        [self stop];
        [self.delegate danmakuViewPlayFinished:self];
//        [self start];
    }else{
